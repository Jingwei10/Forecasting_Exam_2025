...
# Modelvalidering
For at vurdere modellernes forecast-præcision anvendes time series cross-validation. Her evalueres ARIMA-, ETS- og SNaive-modellerne for hver tidsserie med fokus på, hvordan de præsterer på ukendte data.

## Træning og test af modeller
Datasættet opdeles i en træningsperiode (2007–2018) og en testperiode (2019). Modellerne estimeres på træningsdata, og deres præcision vurderes ved at sammenligne forudsigelser for 2019 med de faktiske observationer.
...
## Modellering og forecast
Tre modeller estimeres for hver tidsserie: ARIMA, ETS og SNaive. For hver model genereres et 12-måneders forecast, som sammenlignes med testdata. Præcisionen vurderes med RMSE og MAPE for hver model og tidsserie.
```{r}
# Estimér modeller på træningsdata
...
Forecasts sammenholdes med de faktiske observationer fra 2019. Til vurdering anvendes to centrale metrikker:

- **RMSE (Root Mean Squared Error):** Følsom over for store afvigelser.
- **MAPE (Mean Absolute Percentage Error):** Procentuel præcision, uafhængig af skala.

```{r}
# Sammenlign forecast med testdata
forecast_2019 |> 
  accuracy(test_data) |> 
