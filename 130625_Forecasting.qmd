...
-   **SNaive**: Seasonal Naive – benchmarkmodel, hvor forecast er lig med seneste sæsonværdi\
...
Formålet med dette afsnit er at identificere mønstre og strukturer i datasættet, herunder trends, sæsonvariationer, kønsforskelle og regionale forskelle. Dette gøres gennem en række visualiseringer, der danner grundlag for valg af modeller i de kommende afsnit.

### Udvikling over tid

**Figur 1: Udvikling over tid - Kvinder pr. region**

Figur 1 viser den månedlige arbejdsløshed for kvinder i de fem danske regioner fra 2007 til 2019. Der ses en tydelig sæsonrytme med tilbagevendende stigninger i vintermånederne og fald i sommerperioderne. Niveauet varierer desuden mellem regionerne, hvor Region Hovedstaden generelt ligger højest og Nordjylland lavest. Tendensen peger på strukturelle forskelle samt en overordnet nedadgående trend efter finanskrisen.
...
**Figur 2: Udvikling over tid - Mænd pr. region**

Figur 2 viser arbejdsløsheden blandt mænd i de fem regioner. Niveauet er generelt højere og udsvingene større end for kvinder, og der ses et tydeligt sæsonmønster.
...
**Figur 3: Log-transformeret udvikling i arbejdsløshed hos mænd pr. region** For at håndtere de markante udsving i mænds arbejdsløshedsniveauer er data log-transformeret i figur 3. Dette jævner forskelle og fremhæver relative ændringer over tid.

```{r}
# Kun mænd, alle regioner - med log
data |> filter((kon == "Mænd")) |> 
  autoplot(log(svalue)) +
  labs(
...
**Figur 4: Udvikling i arbejdsløshed pr. region og køn**

Plottet viser udviklingen i arbejdsløshed blandt kvinder og mænd fordelt på regioner. Plottet viser, at mænd har generelt højere ledighed og større sæsonudsving end kvinder, særligt i Region Hovedstaden og Syddanmark. Kvindernes kurver er generelt glattere og mindre volatile.
...
```{r}
# Opsummering per køn og region
deskriptiv <- data |> 
  as_tibble() |>
  group_by(region, kon) |> 
  summarise(
    mean = mean(svalue, na.rm = TRUE),
    sd   = sd(svalue, na.rm = TRUE),
    min  = min(svalue, na.rm = TRUE),
    max  = max(svalue, na.rm = TRUE),
    .groups = "drop"
  )
```
